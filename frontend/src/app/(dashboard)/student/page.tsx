...
import { CalendarDays, BookOpen, Clock, Bell, FileText, CheckCircle } from "lucide-react";
...
// Map mock slots to TimetableGrid's expected shape
...
export default function StudentDashboard() {
  const gridSlots = mockTimetableSlots.map(toGridSlot);
    ...
    <div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-semibold text-white tracking-tight">
          Good morning, {STUDENT_NAME.split(" ")[0]}
        </h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard label="Classes Today" value={mockTodaySlots.length} icon={CalendarDays} color="purple" delay={0.05} />
        <StatCard label="Enrolled Subjects" value={4} icon={BookOpen} color="cyan" delay={0.1} />
        <StatCard
          label="Next Class"
          value={mockTodaySlots[0]?.start_time.slice(0, 5) ?? "—"}
          icon={Clock}
          color="blue"
          delay={0.15}
        />
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {slot.teacher_name} · {slot.room_number}
                  </p>
                </div>
